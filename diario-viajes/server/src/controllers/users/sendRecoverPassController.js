...
const sendRecoverPassController = async (req, res, next) => {
    try {
        ...
        if (users.length > 0) {
            ...
            const emailSubject =
                'Recuperación de contraseña - Diario de Viajes';

            // Cuerpo del email de verificación.
            const emailBody = `
                Hemos recibido una solicitud para recuperar contraseña en tu cuenta de Diario de Viajes. Si no has sido tú ignora este email.
